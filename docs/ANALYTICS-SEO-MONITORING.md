...
4. Kies "Web" als platform, vul `slimzonnig.nl` in als URL
...
### Verificatie stappen voor slimzonnig.nl

1. Ga naar [search.google.com/search-console](https://search.google.com/search-console)
2. Klik "Add property" → kies **URL prefix** → vul `https://slimzonnig.nl` in
...
4. Voeg ook `https://slimzonnig.github.io` toe als property (zolang GitHub Pages URL actief is)
...
Sitemap: https://slimzonnig.github.io/sitemap-index.xml
```

Wanneer het domein slimzonnig.nl live is, update de Sitemap-URL naar:
```
Sitemap: https://slimzonnig.nl/sitemap-index.xml
```

En update ook `site` in `astro.config.mjs` naar `https://slimzonnig.nl`.
...
- [ ] robots.txt URL updaten zodra slimzonnig.nl live is
- [ ] astro.config.mjs `site` updaten naar slimzonnig.nl
